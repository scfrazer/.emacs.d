...
;; RCS:		$Id: vhdl-mode.el,v 33.11 2005/06/22 10:48:17 reto Exp reto $
;; Keywords:	languages vhdl
;; WWW:		http://opensource.ethz.ch/emacs/vhdl-mode.html

(defconst vhdl-version "3.33.5"
  "VHDL Mode version number.")

(defconst vhdl-time-stamp "2005-06-22"
...
;; Emacs 22 changed the behavior of read-file-name

(defun vhdl-original-read-file-name (prompt directory default existing initial)
  (let ((filename (read-file-name prompt directory default existing initial)))
    (if (string= "" filename)
        default
      filename)))
...
search, and an argument indicating ."
  (interactive "p\np")
  (let ((count (or count 1))
	(case-fold-search t)
   ...
   (let ((name (vhdl-resolve-env-variable
		(vhdl-replace-string
		 ...
		 (concat (subst-char-in-string
			 ...
			 " " (user-login-name))))))
     (list (vhdl-original-read-file-name
	    "Write project file: "
