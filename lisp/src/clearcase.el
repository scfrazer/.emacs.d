  ...
  (save-excursion
    (progn
      ...
      (if (= 1 (point))
          ...
          (let* ((pwv (clearcase-ct-cleartool-cmd "pwv" "-short"))
                 (viewtag (progn (string-match "\\(\n\\)" pwv)
                                 (replace-match "" nil nil pwv))))
            (if viewtag
                (progn
                  (forward-line 1)
      ...
      (while (not (eobp))
        (cond
         ...
         ((setq subdir (dired-get-subdir))
          ...
          (setq element-list (nth 2 modified-file-info))
         ...
         ((setq fullpath (dired-get-filename nil t))
          ...
          (when (looking-at
            ...
            (let ((buffer-read-only nil)
                  (gid (match-string 1))
                  (file-size (match-string 2)))
              (goto-char (match-beginning 1))
              (delete-region (point) (match-end 2))
              (cond ((if checkout-list
                         (member fullpath checkout-list)
                       (clearcase-fprop-checked-out fullpath))
                     (if (clearcase-fprop-reserved fullpath)
                         (clearcase-dired-insert-status "CHECKOUT-R" 'clearcase-dired-checkedout-face file-size)
                       (clearcase-dired-insert-status "CHECKOUT-U" 'clearcase-dired-checkedout-face file-size)))
                    ((if hijack-list
                         (member fullpath hijack-list)
                       (clearcase-fprop-hijacked fullpath))
                     (clearcase-dired-insert-status "HIJACK" 'clearcase-dired-checkedout-face file-size))
                    ((when element-list
                       (member fullpath element-list))
                     (clearcase-dired-insert-status "cc-element" 'clearcase-dired-element-face file-size))
                    (t
                     (clearcase-dired-insert-status gid nil file-size)))))))
        (forward-line 1))))
  (message "Reformatting...Done"))

(defun clearcase-dired-insert-status (text face file-size)
  (let ((text-start (point)))
    (insert (format "%-10s %7s" text file-size))
    (when (and face (fboundp 'put-text-property))
      (put-text-property text-start (+ text-start 10) 'face face))))
  ...
  (let* ((true-directory (file-truename directory))
    ...
    (if (and viewtag
        ...
        (let* ((ignore (message "Listing ClearCase modified files..."))
          ...
          (mapc (function
                 (lambda (line)
                   ...
                   (if (string-match "^\\([^ @]+\\)@@[^ ]+?[0-9] " line)
                       (setq element-list (cons (substring line
                                                           (match-beginning 1)
                                                           (match-end 1))
                                                element-list)))))
                line-list)
          ...
          ;; Return the result.
          ;;
          (setq result (list checkout-list hijack-list element-list))))
    result))
...
(defun clearcase-uncheckout-current-buffer (&optional arg)
...
(defun clearcase-uncheckout-dired-files (&optional arg)
...
(defun clearcase-dired-uncheckout-current-dir (&optional arg)
  (interactive "P")
  (if arg
      (let ((clearcase-keep-uncheckouts nil))
...
(defvar clearcase-diff-gui-tool nil
  "*Tool to use for graphical diffs.")
...
(defun clearcase-gui-diff-file-with-version (truename other-version)
  (let* ((other-vxpath (clearcase-vxpath-cons-vxpath (clearcase-vxpath-element-part truename)
         ...
         (gui-name (or clearcase-diff-gui-tool
                       (if clearcase-on-mswindows
                           "cleardiffmrg"
                         "xcleardiff"))))
    (start-process "Diff"
...
(defvar clearcase-update-buffer-scopes nil)
  ...
  (let ((containing-dir (file-name-directory file)))
    ...
    (let ((dir-checkout-needed (not (clearcase-fprop-checked-out containing-dir))))
      ...
      (if (null comment)
          ;; If no comment supplied, go and get one...
          ;;
          (if clearcase-initial-mkelem-comment
              (clearcase-comment-start-entry (file-name-nondirectory file)
            ...
            (lambda (file)
              (clearcase-ct-do-cleartool-command "lshistory"
                                                 ...
                                                 (if (eq mtype 'directory-version)
                                                     (list "-d")
                                                   (list "-fmt" "'%Sd  %-10u  %-20e \"%n\"%[ from ]f %.1Cl%[ ]Tf%[  ]Rf\\n%c'")))
...
(defun clearcase-diff-files (file1 file2)
  ...
  (let ((diff-size  (with-current-buffer "*clearcase*"
                      (buffer-size))))
    (if (zerop diff-size)
...
(defvar clearcase-menu-contents-minimised
  (list "ClearCase"
        ...
        ["Checkout (unreserved)" clearcase-checkout-unreserved-current-buffer
         :keys nil
         :visible (clearcase-file-ok-to-checkout buffer-file-name)]
...
(defvar clearcase-dired-menu-contents-minimised
  (list "ClearCase"
        ...
        ["Checkout file (unreserved)" clearcase-checkout-unreserved-dired-files
         :keys nil
         :visible (and (< (clearcase-dired-mark-count) 2)
                       (clearcase-dired-current-ok-to-checkout))]
        ...
        ["Checkout marked files (unreserved)" clearcase-checkout-unreserved-dired-files
         :keys nil
         :visible (and (>= (clearcase-dired-mark-count) 2)
                       (clearcase-dired-marked-ok-to-checkout))]
        ...
        ["Checkout current dir (unreserved)" clearcase-dired-checkout-unreserved-current-dir
         :keys nil
         :visible (clearcase-dired-current-dir-ok-to-checkout)]
