...
(defun ahg-status-unmark-all ()
  (interactive)
  (ewoc-map (lambda (d) (when (car d) (setcar d nil) t)) ahg-ewoc))
...
the singleton list with the node at point."
  (let ((marked (ewoc-collect ahg-ewoc
                              (if filter (lambda (d)
    ...
    (if (null marked)
      (cond ((eq action-if-empty nil) nil)
            ((eq action-if-empty 'all)
             (ewoc-collect ahg-ewoc (if filter filter 'identity)))
            ((eq action-if-empty 'cur)
             (let ((n (ewoc-locate ahg-ewoc)))
...
the file on the current line."
  ...
  (let ((files
         (if all (ahg-status-get-marked 'all)
           (let ((n (ewoc-locate ahg-ewoc))) (when n (list (ewoc-data n))))))
  ...
  (let ((buf (ahg-get-status-buffer root t))
    ...
    (with-current-buffer buf
      ...
      (let ((ew (ewoc-create 'ahg-status-pp
                             header footer)))
        (ahg-status-mode)
        (set (make-local-variable 'ahg-ewoc) ew)
...
(defvar ahg-window-configuration nil)
...
do nothing."
  ...
  (let ((n (ewoc-nth ahg-ewoc 0)))
    (while (and n (not (string= rev (car (ewoc-data n)))))
      (setq n (ewoc-next ahg-ewoc n)))
    (when n (ewoc-goto-node ahg-ewoc n))))
