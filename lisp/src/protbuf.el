...
(defgroup no-kill nil
  "Protect buffers from being killed."
  :version "21.1"
  :group 'tools)

(define-minor-mode no-kill-mode
  "Minor mode for protecting buffers from being killed."
  :group no-kill
  :init-val nil
  :lighter " NoKill"
  (setq protect-buffer-from-kill-mode no-kill-mode))

(define-key mode-line-mode-menu [no-kill-mode]
  `(menu-item ,(purecopy "No-Kill") no-kill-mode
              :button (:toggle . (bound-and-true-p no-kill-mode))))
