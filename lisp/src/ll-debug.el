...
(defvar ll-debug-print-filename t
  "*Print filename in debug statement (buffer-local)")
...
(defun ll-debug-revert ()
  "Deletes (with confirmation) lines containing the regexp `ll-debug-output-prefix'.
...
(defun ll-debug-get-c++-function-name ()
  (interactive)
  (save-excursion
    (save-match-data
