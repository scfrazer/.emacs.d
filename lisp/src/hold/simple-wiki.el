...
(defgroup simple-wiki ()
  "Edit raw wiki pages." :group 'docs)
...
\\{simple-confluencewiki-mode-map}"

 :em-patterns (list '("\\(\\W\\|^\\)_.*?_" . 0) ; emphasis
                    '("\\W\\*.*?\\*" . 0) ; strong (bold at bol is a bullet list)
                    nil) ; strong emphasis
 :em-strings '("_" . "_")
 :strong-strings '("*" . "*")
 :strong-em-string 'none
 :headlines '(("^[ \t]*\\(h1\\..*\\)$" . 1)
              ("^[ \t]*\\(h2\\..*\\)$" . 1)
              ("^[ \t]*\\(h3\\..*\\)$" . 1)
              ("^[ \t]*\\(h4\\..*\\)$" . 1)
 ...
 :keywords (list
            '("\\(\\W\\|^\\)\\(-.*?-\\)" .
              (2 'simple-wiki-strike-face append))
            '("\\(\\W\\|^\\)\\({[^{].*?}\\)" .
...
(simple-wiki-define-major-mode
 'mymediawiki
 "MyMediaWiki"
 "Simple mode to edit a variation on MediaWiki pages.
\\{simple-mymediawiki-mode-map}"
 ...
 ;; :headlines.......... overwrite `simple-wiki-headline-patterns'
 :keywords (list
            '("\\(\\W\\|^\\)\\(#.*?#\\)" . (2 'simple-wiki-strike-face append))         ;; Strikethough
            '("\\(\\W\\|^\\)\\({{[^{].*?}}\\)" . (2 'simple-wiki-teletype-face append)) ;; Teletype
            ;; To-do lists
