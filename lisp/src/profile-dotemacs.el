  ...
  (let ((totaltime (profile-dotemacs-totaltime results))
    ...
    (while results
      (let* ((current (pop results))
        ...
        (setq benchstr (profile-dotemacs-make-benchstr current percentage))
...
;;   (let* ((col1 (or (color-values col-begin) col-begin))
;;          (col2 (or (color-values col-end) col-end))
;;          (col
;;           (mapcar (lambda (c)
;;                     (round
...
(defun profile-dotemacs-make-benchstr (timings percentage)
