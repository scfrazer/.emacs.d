...
(defun git-simple-refresh ()
  ...
  (interactive)
  (message "Refreshing ...")
  (when (string-prefix-p git-simple-buf-prefix (buffer-name))
        ...
        (while (not (eobp))
          (cond ((looking-at "## \\([a-zA-Z0-9_./-]+\\)[.][.][.]\\(\\([a-zA-Z0-9_-]+\\)/[a-zA-Z0-9_./-]+\\)\\( .+\\)?")
                 (let ((branch (match-string-no-properties 1))
                       (remote-branch (match-string-no-properties 2))
                       (remote (match-string-no-properties 3))
                       (state (match-string-no-properties 4)))
                   (with-current-buffer buf
                     (insert "Branch:    " branch (or state "") "\n")
                     (insert "Tracking:  " remote-branch "\n")
                     (insert "URL:       " (git-simple-get-url remote) "\n")
                     (insert "\n"))))
